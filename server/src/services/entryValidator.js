...
function buildPackageCount(legs) {
  const calendarQty = legs
    .filter((leg) => String(leg.role || "").toLowerCase() === "short_put_calendar")
    .map((leg) => asNumber(leg.qty));
  return Math.max(1, ...calendarQty);
}
...
export function validateEntry(payload, settings) {
  ...
  const packageCount = buildPackageCount(legs);
  const netDebitPerLot = packageCount > 0 ? netDebit / packageCount : netDebit;
  ...
  if (vix <= 0 || vix9d <= 0) {
    elevate("CAUTION", "CAUTION - Enter both VIX and VIX9D to compute the VIX9D/VIX ratio filter.", "VIX-RATIO-PENDING");
  ...
  if (netDebitPerLot < settings.min_net_debit_per_lot) {
    elevate(
      "BLOCKED",
      `SKIP TRADE - Net debit per lot is $${netDebitPerLot.toFixed(2)}. Minimum required is $${settings.min_net_debit_per_lot.toFixed(2)}.`,
      "NET-DEBIT"
    );
  }
  ...
  if (vix > 0) {
    elevate(
      "APPROVED",
      `${vixZone.note} Suggested allocation: ${(allocationPct * 100).toFixed(1)}% of portfolio. Profit target: ${vixZone.profit_target_pct.toFixed(0)}%.`,
      "VIX-ZONE"
    );
  }
  ...
  return {
    ...
    premium_per_contract: netDebitPerLot,
